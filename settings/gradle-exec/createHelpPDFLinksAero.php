...
preg_match("/APLICATION_AERO_MAJOR_VERSION.*?(\\d+)/", $content, $major);
preg_match("/APLICATION_AERO_MINOR1_VERSION.*?(\\d+)/", $content, $minor);
...
file_put_contents($helpinkFile, preg_replace('/\/\/linkAero.*\/\/endlinkAero/s', $result, $helpinkFileContent));
