package com.lib.translate;

import com.customWidget.picker.ProgresExViewTranslateInterface;


public class AutorotationBailOutProgressExTranslate implements ProgresExViewTranslateInterface
{

    @Override
    public String translateCurrent(int current)
    {
        return String.valueOf(current) + " s";
    }

    @Override
    public String translateMin(int min)
    {
        return String.valueOf(min) + " s";
    }

    @Override
    public String translateMax(int max)
    {
        return String.valueOf(max) + " s";
    }

}